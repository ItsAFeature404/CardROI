...
fn add_photo_makes_the_first_upload_primary_and_position_zero() {
    ...
    let photo = repo
        .add_photo(holding_id, &fake_jpeg(400, 300), images_root.path())
        .unwrap();
...
fn get_photo_bytes_reads_the_full_size_file_not_the_small_thumbnail() {
    ...
    let photo = repo
        .add_photo(holding_id, &fake_jpeg(800, 600), images_root.path())
        .unwrap();

    let full_bytes = repo.get_photo_bytes(photo.id, images_root.path()).unwrap();
...
fn uploading_identical_bytes_twice_creates_two_rows_but_writes_the_file_once() {
    ...
    let first = repo
        .add_photo(holding_id, &bytes, images_root.path())
        .unwrap();
    let second = repo
        .add_photo(holding_id, &bytes, images_root.path())
        .unwrap();
...
fn oversized_images_are_capped_to_the_long_edge_limit() {
    ...
    let photo = repo
        .add_photo(holding_id, &fake_jpeg(3000, 1500), images_root.path())
        .unwrap();
...
fn an_undecodable_upload_is_rejected_cleanly_not_a_panic() {
    ...
    let err = repo
        .add_photo(holding_id, b"not an image", images_root.path())
        .unwrap_err();
...
fn deleting_one_of_two_rows_sharing_a_hash_keeps_the_file_deleting_the_last_removes_it() {
    ...
    let photo_a = repo
        .add_photo(holding_a, &bytes, images_root.path())
        .unwrap();
    let photo_b = repo
        .add_photo(holding_b, &bytes, images_root.path())
        .unwrap();
    assert_eq!(count_files_in(images_root.path()), 1);

    repo.delete_photo(photo_a.id, images_root.path()).unwrap();
    assert_eq!(
    ...
    repo.delete_photo(photo_b.id, images_root.path()).unwrap();
    assert_eq!(
        count_files_in(images_root.path()),
        0,
...
fn deleting_the_primary_photo_promotes_the_next_one() {
    ...
    let first = repo
        ...
        .unwrap();
    let second = repo
        .add_photo(holding_id, &fake_jpeg(300, 400), images_root.path())
        .unwrap();
    assert!(first.is_primary);

    repo.delete_photo(first.id, images_root.path()).unwrap();
...
fn deleting_the_only_photo_leaves_no_primary_behind() {
    ...
    let only = repo
        .add_photo(holding_id, &fake_jpeg(400, 300), images_root.path())
        .unwrap();
    repo.delete_photo(only.id, images_root.path()).unwrap();
...
fn deleting_a_holding_cascades_its_photo_rows_and_cleans_up_unreferenced_files() {
    ...
    repo.add_photo(holding_id, &fake_jpeg(400, 300), images_root.path())
        .unwrap();
    repo.add_photo(holding_id, &fake_jpeg(300, 400), images_root.path())
        .unwrap();
    assert_eq!(count_files_in(images_root.path()), 2);

    repo.delete_holding_with_images(holding_id, images_root.path())
        .unwrap();
...
fn deleting_a_holding_does_not_remove_a_file_still_referenced_by_another_holding() {
    ...
    repo.add_photo(holding_a, &bytes, images_root.path())
        .unwrap();
    repo.add_photo(holding_b, &bytes, images_root.path())
        .unwrap();

    repo.delete_holding_with_images(holding_a, images_root.path())
        .unwrap();
...
fn png_uploads_are_accepted_and_normalized_to_jpeg() {
    ...
    let photo = repo
        .add_photo(holding_id, &png_bytes, images_root.path())
        .unwrap();
